...
const Statistics = () => {
  ...
  const users = new Date().getTime().toString().slice(5)
  const earning = new Date().getTime().toString().slice(5)
  const payouts = {
      ...
      {!isLoading ? (
        <SpaceContainer className="d-flex justify-content-around flex-row w-100">
          ...
          <Popover
            ...
            title="Payout"
          >
            <Statistic className="m-4" title="Payouts" value={parseFloat(payouts.total).toFixed(2)} prefix="$" />
          </Popover>
          <Statistic className="m-4" title="Earnings" value={parseFloat(earning).toFixed(2)} prefix="$" />
        </SpaceContainer>
      ) : (
        <SpaceContainer className="d-flex justify-content-around flex-row w-100">
